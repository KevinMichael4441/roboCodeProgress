...
import java.awt.geom.Point2D;									// to find magnitude of distance b/w two points
import static robocode.util.Utils.normalAbsoluteAngle;			// To normalise absolute angles (in radians)
import static robocode.util.Utils.isNear;						// to check if double values are equal
import static robocode.util.Utils.normalRelativeAngleDegrees;	// To normalise relative angles (in degress)
import java.util.Random;										// To randomize

// API help : https://robocode.sourceforge.io/docs/robocode/robocode/Robot.html

/*
 * BigMack - a robot by Kevin, Lini and Marie-Elise
 */
...
public class BigMack extends Robot
{
	...
	double dodgeAmount = 125; // Distance to dodge
	...
	public void linearFiring(double t_oppHeading, double t_oppVelocity)
	{
		...
		// if velocity is LESS than pred location, bullet will fall short => we run loop till they are about equal.
		{
	...
	{
		fireTurn++;
		if (fireTurn % 6 == 0 )		// checking how many bullets Hit every 6 times a shot is fired
		// 6 up and 2 below are arbitrary values found from testing
		{
			if (bulletsMissed > 2)	// changing shoot mode when more than 2 bullets miss
			{
				fireMode++;
			}
			fireTurn = 0;
